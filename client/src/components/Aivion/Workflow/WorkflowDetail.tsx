...
export default function WorkflowDetail() {
  ...
  async function handleStart(e: React.FormEvent) {
    ...
    try {
      ...
      navigate(`/workflow/${id}/runs/${run_id}`);
    } catch {
                ...
                {steps.map((step, i) => (
                  <li key={step.id} className="flex gap-3">
                    <div className="flex w-6 shrink-0 flex-col items-center">
                      <div className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-border-light bg-surface-primary text-[9px] font-bold text-text-secondary">
                        {i + 1}
                      </div>
                      {i < steps.length - 1 && (
                        <div className="my-1 w-px flex-1 bg-border-light" style={{ minHeight: 12 }} />
                      )}
                    </div>
